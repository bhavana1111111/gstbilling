<?php 
    ...
    $q1="SELECT * FROM sales_mast P INNER JOIN cust_mast S ON P.Cust_Id=S.Cust_Id WHERE P.Client_Id='{$clid}'";
...
<div id="main" class="container bg-light rounded bg-white shadow-sm">
    <form class="form-group p-3" method="POST" name="Salesproduct">
		<h4 class="text-primary">Sales Invoice Printing</h4>
        <hr class="mb-4">
        <div class="row ml-1">
            <div class="col">
                <select class="custom-select d-block w-100" name="invn" id="invn" required>
                    <?php
                        ...
                        { 
                    ?>
                    <option value='<?= $row1["Inv_No"]?>'><?= $row1["Inv_No"]?>-<?= $row1["Cust_Name"]?></option>
                    <?php 
                        }
                    ?>
                </select>
