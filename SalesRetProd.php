<?php 
    ...
    $fdt=$_SESSION['fdt'];
    $tdt=$_SESSION['tdt'];
    $q1="SELECT P.Pr_Id,Pr_Name,Pr_Desc,Pr_Rate,sum(Pr_Qty) as SalesQ,sum(Amt) as SalesA,sum(Disc) as SalesD,Gst_Rate,sum(Total_Amt) as SalesT FROM sales_ret_det S INNER JOIN prod_mast P ON S.Pr_Id=P.Pr_id WHERE Srn_No IN (SELECT Srn_No FROM sales_ret_mast WHERE Client_Id='{$clid}' AND Srn_date>='{$fdt}' AND Srn_date<='{$tdt}') GROUP BY P.Pr_Id ORDER BY P.Pr_Id";
    ...
    <div class="table-responsive-sm text-center">
        <table  id="tblCustomers"class="table table-striped text-center" border="2">
            <thead>
                ...
                <tr>
                    <th class="center" colspan="11">ProductWise Sales Return Report</th>
                </tr>
                <tr>
        ...
        <script type="text/javascript">
            function Export() {
                $("#tblCustomers").table2excel({
                    filename: "Productwise Sales Return.xls"
                });
            }
        </script>
    </div>
