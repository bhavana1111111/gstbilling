<?php 
    ...
    $fdt=$_SESSION['fdt'];
    $tdt=$_SESSION['tdt'];
    $q1="SELECT P.Pr_Id,Pr_Name,Pr_Desc,sum(Amt)/sum(Pr_Qty) as Rate,sum(Pr_Qty) as PurchaseQ,sum(Amt) as PurchaseA,sum(Disc) as PurchaseD,Gst_Rate,sum(Total_Amt) as PurchaseT FROM purchase_det S INNER JOIN prod_mast P ON S.Pr_Id=P.Pr_id WHERE Inv_No IN (SELECT Inv_No FROM purchase_mast WHERE Client_Id='{$clid}' AND Inv_date>='{$fdt}' AND Inv_date<='{$tdt}') GROUP BY P.Pr_Id ORDER BY P.Pr_Id";
    ...
    <div class="table-responsive-sm text-center">
        <table  id="tblCustomers"class="table table-striped text-center" border="2">
            <thead>
                ...
                <tr>
                    <th class="center" colspan="11">ProductWise Purchase Report</th>
                </tr>
                <tr>
        ...
        <script type="text/javascript">
            function Export() {
                $("#tblCustomers").table2excel({
                    filename: "Productwise Purchase.xls"
                });
            }
        </script>
    </div>
