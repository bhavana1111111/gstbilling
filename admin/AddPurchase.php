<?php 
	...
	$q1="SELECT * FROM purchase_mast WHERE Client_Id='".$clid."'";
	$q2="DELETE FROM purchase_det_temp";
	...
	if(isset($_POST['submit']))
  	{
  		$q5="SELECT * FROM purchase_mast WHERE Inv_No='".$prev."'";
		...
		else
		{
			...
			$dt=$row5['Inv_Date'];
			$dt1= date('Y-m-d', strtotime($_POST['InvDt']));
			$cur=date('Y-m-d',time());
			print($cur);
...
<div id="main" class="container bg-light rounded bg-white shadow-sm">
	<form class="form-group p-3" method="POST" name="PMaster">
		...
		<div class="row ml-1">
			...
			<div class="m-1 col border">
				<label class="form">Select Supplier name</label>
				<select class="custom-select d-block w-50 mx-sm-3" name="spnm" id="spnm" required>
					<?php
          				$q3="SELECT * FROM supp_mast WHERE Client_id='".$clid."'";
