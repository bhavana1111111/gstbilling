<?php 
    ...
    $fdt=$_SESSION['fdt'];
    $tdt=$_SESSION['tdt'];
    $q1="SELECT S.Cust_Id,Cust_Name,Address,City,State,Pincode,Gstn_No,Mob_No,sum(Final_Amt) as SalesT FROM sales_ret_mast P INNER JOIN cust_mast S ON S.Cust_Id=P.Cust_Id WHERE S.Client_id='{$clid}' AND Srn_date>='{$fdt}' AND Srn_date<='{$tdt}' GROUP BY S.Cust_Id ORDER BY S.Cust_Id";
    ...
    <div class="table-responsive-sm text-center">
        <table  id="tblCustomers"class="table table-striped text-center" border="2">
            <thead>
                ...
                <tr>
                    <th class="center" colspan="10">Dealerwise Sales Return Report</th>
                </tr>
                <tr>
        ...
        <script type="text/javascript">
            function Export() {
                $("#tblCustomers").table2excel({
                    filename: "Dealerwise Sales Return.xls"
                });
            }
        </script>
    </div>
