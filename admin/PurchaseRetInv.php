<?php 
    ...
    $q1="SELECT * FROM purchase_ret_mast P INNER JOIN supp_mast S ON P.Supp_Id=S.Supp_Id WHERE P.Client_Id='{$clid}'";
...
<div id="main" class="container bg-light rounded bg-white shadow-sm">
    <form class="form-group p-3" method="POST" name="Salesproduct">
		<h4 class="text-primary">Purchase Return Invoice Printing</h4>
        <hr class="mb-4">
        <div class="row ml-1">
            <div class="col">
                <select class="custom-select d-block w-100" name="invn" id="invn" required>
                    <?php
                        ...
                        { 
                    ?>
                    <option value='<?= $row1["Prn_No"]?>'><?= $row1["Prn_No"]?>-<?= $row1["Supp_Name"]?></option>
                    <?php 
                        }
                    ?>
                </select>
