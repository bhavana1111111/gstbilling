...
<head>
  <script>
    ...
    {
      if(/^[789]{1}[0-9]{9}$/.test(Customer.Mob_No.value))
      {
        if(/^[1-9]{1}[0-9]{5}$/.test(Customer.Pincode.value))
        {
          if(document.forms["Customer"]["GSTNNo"].value=="")
          {
          ...
          {
            if(/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z]{1}[1-9]{1}[0-9,A-Z]{2}$/.test(Customer.GSTNNo.value))
            {
              return true;
            }
            else
...
<div id="main" class="container bg-light rounded bg-white shadow-sm">	
	<form class="form-group p-3 needs-validation" method="post" name="Customer" onsubmit="return validateForm()">
    ...
    <div>
      ...
      <input type="text" class="form-control" name="GSTNNo" id="GSTNNo" placeholder="Enter your GST No. here" value="">
    </div><br>
    <div>
...
<?php
  if($_POST)
  {
    ...
    else
    {
      ...
      if ($cnt > 9)
      {
        $str =$str."C0".strval($cnt);
      }
      else
      {
        $str =$str."C00".strval($cnt);
      }
      $nm=$_POST['Name'];
      $add=$_POST['Address'];
      $st=$_POST['state'];
      ...
      $gsn=$_POST['GSTNNo'];
      $mbn=$_POST['Mob_No'];
      $q2="INSERT into cust_mast VALUES('".$str."','".$clid."','".$nm."','".$add."','".$ct."','".$st."',".$pc.",'".$gsn."',".$mbn.")";
